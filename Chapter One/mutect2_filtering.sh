...
for vcf in /ph-users/rugare/cancer_data/serum/SRR*/*mut2.vcf
do
...
#Remove variants of low quality
       cat `dirname $vcf`/db_filtered/annotated_`basename $vcf` | java -Xmx8G -jar SnpSift.jar filter "( DP > 20 )" > `dirname $vcf`/db_filtered/tmp2_`basename $vcf`
#Database filtering
#gnomad
